...
export const metadata: Metadata = {
    ...
    description: "Explore our range of financial Investment Advisory services designed to meet your investment needs.",
};
  ...
  return (
    <main className='w-full h-full px-5 lg:px-10 xl:px-24 py-14'>
      <div className='flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 -mt-4'>
        <h1 className='text-xl font-medium shadow p-4 inline-block rounded-xl'>Investment Advisory Services</h1>
        <ServicesSearchBar 
          q={query}
          type={type}
        />
      ...
      <Button asChild variant={'link'} className='px-0 text-xs opacity-75'>
         <Link href={"/ra-services"} className="hover:!text-legacisGreen "> 
            Explore RA Services <ArrowRight size={16} />
         </Link>
      </Button>
      
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-stretch gap-4 xl:gap-8 2xl:gap-12 mt-16">
         {services.length === 0 && (
               ...
               return (
                  <Link 
                     ...
                     <p className="text-base text-violet-800 mb-4">{service.tag}</p>
                      ...
                      <div className="flex items-baseline gap-2 mb-4 text-violet-700 dark:text-violet-300">
                          <span className="text-4xl font-bold font-urbanist">{service.serviceClass}</span>
                      </div>
                     )}
