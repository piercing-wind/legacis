...
export const metadata: Metadata = {
    ...
    description: "Explore our range of financial Research Advisory services designed to meet your investment needs.",
};
...
const Page = async ({ searchParams }: PageProps) => {
  ...
  // Fetch all services except IA services
  const services = (await findServices(undefined, investment_advisory_services))
   .filter((service) => {
     if (type && type !== "ALL") {
  ...
  return (
    <main className='w-full h-full px-5 lg:px-10 xl:px-24 py-14'>
      <div className='flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 -mt-4'>
        <h1 className='text-xl font-medium shadow p-4 rounded-xl'>Research Advisory Services</h1>
        <ServicesSearchBar 
          q={query}
          type={type}
        />
      ...
      <Button asChild variant={'link'} className='px-0 text-xs opacity-75'>
         <Link href={"/ia-services"} className="hover:!text-legacisGreen "> 
            Explore IA Services <ArrowRight size={16} />
         </Link>
      </Button>
