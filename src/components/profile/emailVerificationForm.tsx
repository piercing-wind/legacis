...
import { CheckCircle2, CheckIcon, EyeIcon, EyeOffIcon, Info, X } from "lucide-react";
...
export default function EmailVerificationForm({className}:{className ?:string}) {
...
async function onSubmit(values: z.infer<typeof EmailVerificationSchema>) {
      ...
      startTransition(() => {
         sendOTP({identifier, verificationType : 'EMAIL_VERIFY'})
          .then((res) => {
            ...
            dispatch(setVerificationType('EMAIL_VERIFY'));
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className={cn("space-y-4 rounded-xl max-w-md w-full shadow-legacisPurple/20 px-8 p-8 flex flex-col bg-white border dark:bg-neutral-800", className)}>
        <div className="space-y-4 gap-x-8 w-full">
         <div className="flex items-center justify-between">
            <h5 className="pb-4 text-2xl font-medium">Verify Your Email</h5>
